...
func (s *server) Node2(ctx context.Context, msg *pb.ReqSaveData) (*pb.ResSaveData, error) {

	// Ingresa al DATA.txt de NameNode
	f2, err := os.OpenFile("DataNode2/DATA.txt", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
	if err != nil {
		panic(err)
	}
	defer f2.Close()
...
func (s *server) Getid2(ctx context.Context, msg *pb.ReqData) (*pb.ResData, error) {
	file, err := os.Open("DataNode2/DATA.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
...
func (s *server) ShutDown(ctx context.Context, msg *pb.ReqShutDown) (*pb.ResShutDown, error) {
	go shutDownNode()
	return &pb.ResShutDown{Data: "Cerrando DataNode2"}, nil
}

// ############### Main ###############

func main() {
	if _, err := os.Stat("DataNode2/DATA.txt"); err == nil {
		e := os.Remove("DataNode2/DATA.txt")
		if e != nil {
			log.Fatal(e)
		}
	}
	...
	// Se crea DATA.txt
	f, err := os.Create("DataNode2/DATA.txt")
	if err != nil {
		fmt.Println(err)
	}
	defer f.Close()
